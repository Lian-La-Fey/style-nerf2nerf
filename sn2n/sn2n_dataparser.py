...
@dataclass
class NerfstudioData2ParserConfig(NerfstudioDataParserConfig):
...
class Nerfstudio2(Nerfstudio):
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        print('DEBUG using custom dataparser')
        ...
        for frame in frames:
            ...
            if "file_path_second" in frame:
                image_second_filepath = Path(frame["file_path_second"])
                image_second_fname = self._get_fname(
                    image_second_filepath,
                    data_dir,
                    #downsample_folder_prefix="masks_",
                )     
                image_second_filenames.append(image_second_fname)
            if "tag_path" in frame:
                tag_filepath = Path(frame["tag_path"])
                tag_fname = self._get_fname(
                    tag_filepath,
                    data_dir,
                    #downsample_folder_prefix="masks_",
                )
                tag_filenames.append(tag_fname)           
        ...
        """
        assert len(image_second_filenames)  == 0 or (len(image_second_filenames) == len(image_filenames))
        assert len(tag_filenames)  == 0 or (len(tag_filenames) == len(image_filenames))
        ...
        image_second_filenames = [image_second_filenames[i] for i in indices] if len(image_second_filenames) > 0 else []
        tag_filenames = [tag_filenames[i] for i in indices] if len(tag_filenames) > 0 else []
        ...
        dataparser_outputs = DataparserOutputs(
            ...
            metadata={
                ...
                "image_second_filenames": image_second_filenames if len(image_second_filenames) > 0 else None,
                "tag_filenames": tag_filenames if len(tag_filenames) > 0 else None,
                **metadata,
            },
        )
